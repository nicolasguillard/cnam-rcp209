...
def perform_deconvolution(
    ...
    """
    for i, module in zip(range(idx_layer, -1, -1), reversed(cnn_module[:idx_layer+1])):
        if verbose:
...
def deconvolution(
    ...
    """
    assert hasattr(cnn_model, "features"), "conv_model must have a attribute 'features' refering to CNN features part"
    cnn_features = cnn_model.features

    # Normalizing idx_layer
    if idx_layer == None:
        idx_layer = len(cnn_features) - 1
    elif idx_layer < 0:
        idx_layer += len(cnn_features)
    
    # check if idx_layer is coherent with cnn_features
    assert (0 <= idx_layer) and (idx_layer < len(cnn_features)), \
    ...
    if clean_feature_map:
        ...
        if isinstance(cnn_features[idx_layer], nn.MaxPool2d):
